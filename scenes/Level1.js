...
class Level1 extends Phaser.Scene {
    ...
    initLevel(level) {
        ...
        if (level == 0) {
            this.score = gameLevels.arr[level].score;
        } else {
            this.score = score;
        }

        this.topScore = localStorage.getItem(gameOptions.localStorageName) == null ? 0 : localStorage.getItem(gameOptions.localStorageName);
        this.scoreText = this.add.text(game.config.width / 16, game.config.height / 25, "SCORE:" + this.score, { fontSize: '40px', fill: '#FFF' });

        levelText = this.add.text(game.config.width / 1.6, game.config.height / 25, gameLevels.arr[level].levelNumber, { fontSize: '40px', fill: '#FFF' });
        ...
        this.ball.body.gravity.y = gameLevels.arr[level].gravity;
        ...
        this.ball.setSize(game.config.height / 2, game.config.width / 2, false)
        let platformX = this.ball.x;
        for (let i = 0; i < 10; i++) {
            let platform = this.platformGroup.create(platformX, game.config.height / 4 * 3 + Phaser.Math.Between(gameLevels.arr[level].platformHeightRange[0], gameLevels.arr[level].platformHeightRange[1]), "ground");
            platform.setOrigin(0.5, 1);
            platform.setImmovable(true);
            platform.displayWidth = Phaser.Math.Between(gameLevels.arr[level].platformLengthRange[0], gameLevels.arr[level].platformLengthRange[1]);
            platform.displayHeight = game.config.height / 15;
            platformX += Phaser.Math.Between(gameLevels.arr[level].platformDistanceRange[0], gameLevels.arr[level].platformDistanceRange[1]);
    ...
    update() {
        ...
        // var isCollided = this.physics.overlap(this.platformGroup, this.ball, function () {
    ...
    collision() {
        this.ball.setVelocityY(gameLevels.arr[this.currentLevel].upVelocity);
        if (this.hitCount == 0) {
            this.platformGroup.getChildren().forEach(function (platform) {
                if (platform.getBounds().right < 0) {
                    this.isCorrectJump = true;
    ...
    arr: [
        {
            ...
            upVelocity: -700,
            speed: 650,
            platformDistanceRange: [100, 250],
            platformHeightRange: [-50, 50],
            platformLengthRange: [60, 100],
            winingScore: 5
        },
        {
            score: 0,
